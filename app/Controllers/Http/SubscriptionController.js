...
class SubscriptionController {
  ...
  async userSubscriptions({ request, response, auth }) {
    ...
    if (subscriptions.size() === 0) {
      return response.status(404).json({
        success: false,
        data: { message: "You do not have any subscriptions yet." },
      });
    }
    return response
      .status(200)
  ...
  async userSubscription({ request, response, auth, params }) {
    const subscription = await Subscription.query()
      .where({ id: params.subscription_id, user_id: auth.user.id })
      .first();
  ...
  async cancelSubscription({ request, response, auth, params }) {
    if (request.match(["/api/cancelSubscription"])) {
      const rules = {
      ...
      var id = request.input("subscription_id");
    } else if (request.match(["/api/subscriptions/:subscription_id/cancel"])) {
    ...
    if (
      ...
      return response.status(405).json({
        success: false,
        data: {
          message: "Subscription already paid for or awaiting approval.",
        },
      });
    }
    ...
    return response.status(200).json({
      success: true,
      data: { message: "Subscription successfully cancelled." },
    });
  }

  async allSubscriptions({ request, response, auth }) {
    if (auth.user.id != 1) {
      return response
        .status(403)
